...
def see_scene_panel():
    ...
    当前场景信息面板
    """
    title_text = text_loading.get_text_data(text_loading.STAGE_WORD_PATH, "75")
...
def change_scene_character_list_panel() -> list:
    ...
    input_s = cmd_button_queue.option_int(
        cmd_button_queue.CHANGE_SCENE_CHARACTER_LIST,
        cmd_column=5,
        askfor=False,
        cmd_size="center",
    )
...
def jump_character_list_page_panel() -> str:
    ...
    角色列表页面跳转控制面板
    """
    message_text = text_loading.get_text_data(text_loading.MESSAGE_PATH, "32")
    name_list_max = int(game_config.in_scene_see_player_max)
...
def in_scene_button_panel(start_id: int) -> list:
    ...
    era_print.line_feed_print()
    input_s = cmd_button_queue.option_int(
        cmd_list=cmd_button_queue.in_SCENE_LIST1,
        cmd_column=9,
        askfor=False,
        cmd_size="center",
        start_id=start_id,
