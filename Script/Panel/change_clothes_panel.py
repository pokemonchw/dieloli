...
def see_character_wear_clothes_info(character_id: int):
    ...
    character_id -- 角色id
    """
    scene_info = text_loading.get_text_data(constant.FilePath.STAGE_WORD_PATH, "79")
...
def see_character_wear_clothes(character_id: int, change_button: bool):
    ...
    """
    character_clothing_data = cache_contorl.character_data[character_id].clothing
    ...
    for clothing_text in clothing_text_data:
        ...
        if clothing_text_data[clothing_text] == "None":
            draw_text = (
                ...
                + ":"
                + text_loading.get_text_data(constant.FilePath.STAGE_WORD_PATH, "117")
            )
        else:
            ...
            for tag_text in clothing_text_data[clothing_text]:
                now_tag_text_index = text_handle.get_text_index(tag_text)
                if now_tag_text_index < tag_text_index:
                    draw_text += " " + tag_text + " " * (tag_text_index - now_tag_text_index)
                else:
                    draw_text += " " + tag_text
        if change_button:
...
def see_character_wear_clothes_cmd(start_id: int) -> str:
    ...
    yrn = cmd_button_queue.option_int(
        constant.CmdMenu.SEE_CHARACTER_WEAR_CHOTHES, id_size="center", askfor=False, start_id=start_id,
    )
    return yrn


def see_character_clothes_panel(character_id: str, clothing_type: str, max_page: int) -> list:
    """
    用于查看角色服装列表的面板
    Keyword arguments:
    character_id -- 角色id
    ...
    character_clothing_data = cache_contorl.character_data[character_id].clothing[clothing_type]
    clothing_text_data = {}
    tag_text_index = 0
    ...
    for i in range(now_page_start_id, now_page_end_id):
        ...
        clothing_text = clothing_data["Evaluation"] + clothing_data["Tag"] + clothing_data["Name"]
        clothing_text_data[clothing_text] = {}
        for tag in clothing.clothing_tag_text_list:
            tag_text = clothing.clothing_tag_text_list[tag] + str(clothing_data[tag])
            clothing_text_data[clothing_text][tag_text] = 0
    ...
    for clothing_text in clothing_text_data:
        ...
        for tag_text in clothing_text_data[clothing_text]:
            ...
            if now_tag_text_index < tag_text_index:
                ...
                draw_text += " " + tag_text
        if i == pass_id:
            draw_text += " " + text_loading.get_text_data(constant.FilePath.STAGE_WORD_PATH, "125")
        id_info = cmd_button_queue.id_index(i)
        cmd_text = id_info + draw_text
        input_s.append(f"{i}")
        py_cmd.pcmd(cmd_text, i, None)
...
def see_character_clothes_info(character_id: str):
    ...
    character_id -- 角色id
    """
    scene_info = text_loading.get_text_data(constant.FilePath.STAGE_WORD_PATH, "101")
    era_print.little_title_print(scene_info)
    character_info = attr_text.get_character_abbreviations_info(character_id)
    era_print.normal_print(character_info)
...
def see_character_clothes_cmd(start_id: int, now_clothing_type: str) -> str:
    ...
    next_type_text = [clothing.clothing_type_text_list[clothing_type_list[next_type_id]]]
    cmd_list = up_type_text + cmd_list + next_type_text
    yrn = cmd_button_queue.option_int(
        None, 5, cmd_size="center", askfor=False, start_id=start_id, cmd_list_data=cmd_list,
    )
    return yrn
...
def ask_see_clothing_info_panel(wear_clothing_judge: bool) -> str:
    ...
    cmd_data = text_loading.get_text_data(
        constant.FilePath.CMD_PATH, constant.CmdMenu.ASK_SEE_CLOTHING_INFO_PANEL,
    ).copy()
...
def see_clothing_info_panel(
    ...
    clothing_id -- 服装id
    """
    era_print.little_title_print(text_loading.get_text_data(constant.FilePath.STAGE_WORD_PATH, "126"))
    clothing_data = cache_contorl.character_data[character_id].clothing[clothing_type][clothing_id]
    info_list = []
    clothing_name = clothing_data["Name"]
    if wear_clothing_judge:
        clothing_name += " " + text_loading.get_text_data(constant.FilePath.STAGE_WORD_PATH, "125")
    info_list.append(text_loading.get_text_data(constant.FilePath.STAGE_WORD_PATH, "128") + clothing_name)
    clothing_type_text = clothing.clothing_type_text_list[clothing_type]
    info_list.append(
        text_loading.get_text_data(constant.FilePath.STAGE_WORD_PATH, "129") + clothing_type_text
    )
    evaluation_text = (
        text_loading.get_text_data(constant.FilePath.STAGE_WORD_PATH, "131") + clothing_data["Evaluation"]
    )
    info_list.append(evaluation_text)
    era_print.list_print(info_list, 3, "center")
    era_print.son_title_print(text_loading.get_text_data(constant.FilePath.STAGE_WORD_PATH, "130"))
    tag_text_list = []
    for tag in clothing.clothing_tag_text_list:
    ...
    era_print.son_title_print(text_loading.get_text_data(constant.FilePath.STAGE_WORD_PATH, "127"))
...
def see_clothing_info_ask_panel(wear_clothing_judge: bool) -> str:
    ...
    era_print.line_feed_print()
    cmd_data = text_loading.get_text_data(
        constant.FilePath.CMD_PATH, constant.CmdMenu.SEE_CLOTHING_INFO_ASK_PANEL,
    ).copy()
    if wear_clothing_judge:
        del cmd_data["1"]
    else:
        del cmd_data["2"]
    cmd_list = list(cmd_data.values())
    return cmd_button_queue.option_int(None, 4, cmd_size="center", cmd_list_data=cmd_list)
