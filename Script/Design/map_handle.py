...
def print_map(map_path: list) -> list:
    ...
    for map_x_list_id in range(len(map_y_list)):
        ...
        era_print.normal_print(
            text_handle.align(map_x_list + cmd_list_str, "center", True), rich_text_judge=False,
        )
        i = 0
...
def get_path_finding(map_path_str: str, now_node: str, target_node: str) -> (str, list):
    ...
    if now_node == target_node:
        ...
        return (
            "",
            cache_contorl.map_data[map_path_str]["SortedPath"][now_node][target_node],
        )
...
def sort_scene_character_id(scene_path_str: str):
    ...
    new_scene_character_intimate_data = sorted(
        now_scene_character_intimate_data.items(), key=lambda x: (x[1], -int(x[0])), reverse=True,
    )
