...
def init_character_interest():
    ...
    """
    interest_list = []
    language_skills = text_loading.get_game_data(
        constant.FilePath.LANGUAGE_SKILLS_PATH
    )
    interest_list += list(language_skills.keys())
    knowledge_data = text_loading.get_game_data(
    ...
    for character in cache_contorl.character_data:
        ...
        for interest in now_interest_list:
            if interest != now_interest_list[-1]:
                now_interest_average = now_interest_value_max / len(
                    now_interest_list
                )
                now_inter_value = now_interest_average * random.uniform(
                    0.75, 1.25
                )
