...
def judge_datetime_solar_period(now_time: int) -> (bool, int):
    ...
    if (
        new_time.year == now_time.year
        and new_time.month == now_time.month
        and new_time.day == now_time.day
    ):
...
def get_sun_time(old_time: int) -> int:
    ...
    if (
        ...
        for _ in range(0, 1439):
            ...
            cache.sun_phase[now_unix_date_str][now_unix_date.hour][
                now_unix_date.minute
            ] = get_sun_phase_for_sun_az(now_az)
...
def get_temperature(now_time: float) -> int:
    """
    根据时间计算气温
    Keyword arguments:
    now_time -- 当前unix时间戳
    ...
    now_date_str = f"{now_date.year}/{now_date.month}/{now_date.day}"
    if now_date_str in cache.temperature_data:
