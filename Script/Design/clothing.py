...
def creator_clothing(clothing_tem_id:int) -> game_type.Clothing:
    ...
    clothing_data.price = sum([clothing_data.__dict__[x] for x in clothing_data.__dict__ if isinstance(clothing_data.__dict__[x],int)])
    clothing_data.cleanliness = 100
    clothing_data.evaluation = game_config.config_clothing_evaluate_list[math.floor(clothing_data.price / 480) - 1]
...
def get_clothing_name_data(clothings_data: Dict[int,Dict[uuid.UUID,game_type.Clothing]]) -> Dict[int,Dict[int,Dict[uuid.UUID,int]]]:
    """
...
def get_clothing_price_data(clothings_data: Dict[int,Dict[uuid.UUID,game_type.Clothing]]) -> Dict[int,Dict[uuid.UUID,int]]:
    """
    获取每个类型的服装价值数据
    Keyword arguments:
...
def get_clothing_collocation_data(
    ...
    clothing_collocation_type_data = game_config.config_clothing_collocational_data[now_clothing_data.tem_id]
    for collocation_type in clothing_data:
        if collocation_type not in clothing_collocation_type_data:
            if collocation_type == now_clothing_data.wear:
                continue
            usually_collocation = get_appoint_type_clothing_top(now_clothing_data,clothing_data,clothing_price_data,collocation_data)
            ...
            if now_collocation_data.collocational == 1:
                precedence_list = [now_add_clothing_data.tem_id for now_add_clothing_data in now_collocation_list]
                now_clothing_name_data = clothing_name_data[clothing_type]
                precedence_collocation = get_appoint_names_clothing_top(precedence_list,now_clothing_name_data)
                if precedence_collocation != "None":
                    collocation_data[now_collocation_data.clothing_type] = precedence_collocation
                    collocation_data["Price"] += clothing_price_data[now_collocation_data.clothing_type][precedence_collocation]
                else:
                    usually_collocation = get_appoint_type_clothing_top(now_clothing_data,clothing_data,clothing_price_data,collocation_data)
                    if usually_collocation != "None":
                        collocation_data[collocation_type] = usually_collocation
...
def get_appoint_names_clothing_top(appoint_name_list: List[int], clothing_type_name_data: Dict[int,Dict[uuid.UUID,int]]) -> uuid.UUID:
    """
...
def get_appoint_type_clothing_top(
    ...
    for new_clothing in clothing_type_data_list:
        ...
        if not judge_clothing_collocation(
            ...
            continue
        for collocation_type in collocation_data.__dict__:
...
def judge_clothing_collocation(
    old_clothing_data: game_type.Clothing, new_clothing_data: game_type.Clothing,
) -> bool:
    """
    判断两件服装是否能够进行搭配
    Keyword arguments:
    ...
    bool -- 搭配校验
    """
    old_clothing_data_restrict_data = game_config.config_clothing_collocational_data[old_clothing_data.tem_id]
    new_clothing_data_restrict_data = game_config.config_clothing_collocational_data[new_clothing_data.tem_id]
    if len(old_clothing_data_restrict_data[new_clothing_data.wear]):
        ...
        if old_judge.collocational == 2:
            ...
            if judge:
                return 0
    if len(new_clothing_data_restrict_data[old_clothing_data.wear]):
