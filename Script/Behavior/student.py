...
from Script.Design import game_time, map_handle,settle_behavior,character_behavior,character_move
from Script.Behavior.Action import default_action, student_action


@character_behavior.add_behavior("Student",constant.CharacterStatus.STATUS_ARDER)
def arder_behavior(character_id: int):
    ...
    if now_time_slice["InCourse"]:
        ...
        if character_data.position != character_data.classroom:
            _,_,move_path,move_time = character_move.character_move(character_id,character_data.classroom)
...
@character_behavior.add_behavior("Student",constant.CharacterStatus.STATUS_REST)
